...
for k = 1:loop     %%循环loop次取平均
    ...
    for snr = 1:length(SNR)
        ...
        [V,U] = leakage_minimization_K_user(H(:,1:M,:,:), V, iter_leakmin); %迭代计算U，V
        R_without_AS(snr,k) = rate_K_user_MIMO(U, H(:,1:M,:,:), V, xigma);%计算总速率（bits/s/Hz)
        ...
        while delta > 10^(-2)
            ...
            for i =  1:Q
                ...
                [V,U] = leakage_minimization_K_user(H_AS, V, iter_leakmin); %迭代计算U，V
                min_SINR(i) = abs(min_chordal_distance(U, H_AS, V));
            end
        ...
        [V,U] = leakage_minimization_K_user(H_AS, V, iter_leakmin); %迭代计算U，V
        R_CEO(snr,k) = rate_K_user_MIMO(U, H_AS, V, xigma);
    end
    k
end
display(['Total Time :']);
