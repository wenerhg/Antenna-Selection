% rate_K_user(U, H, V, xigma) computes the data rate of a real K-user IC. 
% The inputs are the K zeroforcers U, the K^2 MrxMt channels H, and the K
% beamformers V.

function R = rate_K_user(U, H, V,xigma)
...
J = zeros(d, (K-1)*d, K); % initialize the interference matrix
  for i = 1:K
        ...
        for j = [1:i-1 i+1:K]
             J(:,(j-1)*d+1:j*d,i) = U(:,:,i)'*H(:,:,i,j)*V(:,:,j); %each term is one interference block
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
